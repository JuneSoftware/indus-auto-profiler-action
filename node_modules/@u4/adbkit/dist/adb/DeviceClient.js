...
var _DeviceClient_extra;
Object.defineProperty(exports, "__esModule", { value: true });
const adbkit_monkey_1 = require("@u4/adbkit-monkey");
const adbkit_logcat_1 = __importDefault(require("@u4/adbkit-logcat"));
const sync_1 = __importDefault(require("./sync"));
const stat_1 = __importDefault(require("./proc/stat"));
const host_1 = require("./command/host");
const hostCmd = __importStar(require("./command/host-transport"));
const host_serial_1 = require("./command/host-serial");
const utils_1 = __importDefault(require("./utils"));
const Scrcpy_1 = __importDefault(require("./thirdparty/scrcpy/Scrcpy"));
const Minicap_1 = __importDefault(require("./thirdparty/minicap/Minicap"));
const STFService_1 = __importDefault(require("./thirdparty/STFService/STFService"));
const promise_duplex_1 = __importDefault(require("promise-duplex"));
const protocol_1 = __importDefault(require("./protocol"));
const serviceCall_1 = __importDefault(require("./command/host-transport/serviceCall"));
const DeviceClientExtra_1 = __importDefault(require("./DeviceClientExtra"));
const DevicePackage_1 = __importDefault(require("./DevicePackage"));
const get_port_1 = __importDefault(require("get-port"));
const debug = utils_1.default.debug('adb:client');
const NoUserOptionError = (err) => err.message.indexOf('--user') !== -1;
class DeviceClient {
    constructor(client, serial, options) {
        this.client = client;
        this.serial = serial;
        _DeviceClient_extra.set(this, void 0);
        options = options || {};
    ...
    async getSerialNo() {
        ...
        return new host_serial_1.GetSerialNoCommand(conn).execute(this.serial);
    }
    /**
    ...
    async getDevicePath() {
        const conn = await this.connection();
        return new host_serial_1.GetDevicePathCommand(conn).execute(this.serial);
    }
    /**
    ...
    async getState() {
        const conn = await this.connection();
        return new host_serial_1.GetStateCommand(conn).execute(this.serial);
    }
    /**
     ...
     * @returns a PsEntry array
     */
    async callServiceRaw(serviceName, code, ...args) {
        const transport = await this.transport();
        return new serviceCall_1.default(transport, this.options).execute(serviceName, code, args);
    }
    /**
     ...
     */
    async getIpAddress(iface = 'wlan0') {
    ...
    async forward(local, remote) {
        const conn = await this.connection();
        return new host_serial_1.ForwardCommand(conn).execute(this.serial, local, remote);
    }
    /**
     ...
     */
    async tryForwardTCP(remote, preferedPort) {
        ...
        const freePort = await (0, get_port_1.default)();
    ...
    async removeForward(local) {
        const conn = await this.connection();
        return new host_serial_1.KillForwardCommand(conn).execute(this.serial, local);
    }
    /**
    ...
    async listForwards() {
        const conn = await this.connection();
        return new host_serial_1.ListForwardsCommand(conn).execute(this.serial);
    }
    /**
    ...
    async transport() {
        const conn = await this.connection();
        await new host_1.HostTransportCommand(conn).execute(this.serial);
        return conn;
    }
    /**
    ...
    async execOut(command, encoding) {
        const duplex = new promise_duplex_1.default(await (this.exec(command)));
        if (encoding) {
     ...
     * Testing only
     */
    async openLocal2(path, debugCtxt = '') {
        const transport = await this.transport();
        const data = path.includes(':') ? path : `localfilesystem:${path}`;
        const duplex = new promise_duplex_1.default(transport.parser.raw());
        await duplex.write(protocol_1.default.encodeData(data));
        await utils_1.default.waitforReadable(duplex, 0, `openLocal2 ${data} - ${debugCtxt}`);
        const code = await duplex.read(4);
        if (!code.equals(protocol_1.default.bOKAY)) {
            if (code.equals(protocol_1.default.bFAIL))
                throw await transport.parser.readError();
     ...
     */
    async openMonkey(port = 1080) {
        const tryConnect = async (times) => {
            try {
                const stream = await this.openTcp(port);
                const client = adbkit_monkey_1.Monkey.connectStream(stream);
                return client;
            }
            catch (err) {
                if ((times -= 1)) {
    ...
    async openLogcat(options = {}) {
        ...
        return adbkit_logcat_1.default.readStream(stream, { fixLineFeeds: false });
    }
    /**
    ...
    async waitForDevice() {
        const conn = await this.connection();
        return new host_serial_1.WaitForDeviceCommand(conn).execute(this.serial);
    }
    /**
     ...
     */
    scrcpy(options) {
        const scrcpy = new Scrcpy_1.default(this, options);
        return scrcpy;
    }
    /**
     ...
     */
    minicap(options) {
        const minicap = new Minicap_1.default(this, options);
        return minicap;
    }
    /**
     ...
     */
    STFService(options) {
        const service = new STFService_1.default(this, options);
        return service;
    }
    /**
     * get extra fucntions
     ...
     */
    get extra() {
        if (!__classPrivateFieldGet(this, _DeviceClient_extra, "f")) {
            __classPrivateFieldSet(this, _DeviceClient_extra, new DeviceClientExtra_1.default(this), "f");
        }
        return __classPrivateFieldGet(this, _DeviceClient_extra, "f");
    }
    /**
    ...
    async listPackages(options) {
        ...
        return packages.map(m => new DevicePackage_1.default(this, m[1]));
    }
}
exports.default = DeviceClient;
_DeviceClient_extra = new WeakMap();
